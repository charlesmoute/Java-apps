 ...
 * @author Charles
 * <p>
 * 	DES est l'implementation de l'algorithme de cryptographie portant le meme nom.
 *  <strong>NB</strong> Dire qu'un parametre tient sur x bits ceci revient a dire que sa representation binaire
 *  tient sur x bits. Exemple: la chaine "abcd" tient sur 32bits si les caracteres sont codes en ASCII. 
 * </p>
 */
public class DES {

	/**
	 * <p>Chiffre le message avec la cle key et retourne le chiffre.</p>
	 * @param message Message a chiffrer sous forme binaire adaptee.
	 * @param key Cle a utiliser pour le chiffremment. Cle pouvant tenir sur 56 bits
	 * @return Le message chiffre en binaire, si les parametres en entree sont corrects sinon null.
	 * @throws Exception Erreur lancee lorsque le code ne peut s'executer correctement.
	 * @see DESUtil#messageToDESBinaryString(String)
	 */
	public static final String encrypt(String message,String key) throws Exception{
		...
		for(int numMessage=0;numMessage<subMessages.size();numMessage++){
			...
			String aux;
			int i= 0,splitPosition=Constant.THIRTYTWO_BIT;
	...
	public static final String decrypt(String message,String key) throws Exception{
		...
		for(int numMessage=0;numMessage<subMessages.size();numMessage++){
			String subMessage = subMessages.get(numMessage);
			String aux;
			int i= 16,splitPosition=Constant.THIRTYTWO_BIT;
			aux = DESUtil.applyPermutationTable(subMessage, DESConstant.IP);			
			String Li,Ri ;
	...
	public static final ArrayList<String> genereateKeySet(String key) throws Exception {
		...
		if(!Binary.isBinaryRepresentation(key) || key.length()!=Constant.SIXTYFOUR_BIT)
	...
	public static final String f(String param,String key) throws Exception{
		...
		if(param.length()!=Constant.THIRTYTWO_BIT) throw new Exception("Le premier parametre de f ne tient pas sur 32 bits");
		
		if(!Binary.isBinaryRepresentation(key)) throw new Exception("La cle partielle utilisee dans f n'est pas sur sa forme binaire");
		if(key.length()!=Constant.FORTYEIGHT_BIT) throw new Exception("La cle partielle utilisee dans f ne tient pas sur 48 bits");
		
		String result = "" ;
		String paramExpansion = DESUtil.applyPermutationTable(param, DESConstant.EXPANSION);
		String aux = Binary.xor(paramExpansion, key);
		int j=0, incr = Constant.SIX_BIT;		
		for(int i=1;i<=8;i++){
		...
		if(result.length()!=Constant.THIRTYTWO_BIT)
	...
	public static final String sboxes(int numSBoxe,String param) throws Exception{
		...
		if(!Binary.isBinaryRepresentation(param)|| param.length()!=Constant.SIX_BIT )
	...
	public static final ArrayList<String> split(String message) throws Exception{
		...
		ArrayList<String> list = new ArrayList<String>();
		int i=0, incr = Constant.SIXTYFOUR_BIT;
	...
	public static void main(String[] arg) throws Exception{
		...
		String message="4E6F772069732074";
		...
		String binary = DESUtil.messageToDESBinaryString(message);
		System.out.println("binary = "+binary);
		System.out.println();
		
		String crypted = DES.encrypt(binary, key);
		System.out.println("crypted = "+crypted);
		System.out.println();
		
		String valueCrypted = DESUtil.binaryStringToReadableMessage(crypted);
		System.out.println("valueCrypted = "+valueCrypted);
		System.out.println();
		
		String binaireValue = DESUtil.messageToDESBinaryString(valueCrypted);
		System.out.println("binaireValue = "+binaireValue);
		System.out.println();
		
		System.out.println("binaireValue==crypted = "+(binaireValue.compareTo(crypted)==0));
		System.out.println();
		
		String discrypted = DES.decrypt(binaireValue, key);
		System.out.println("discrypted = "+discrypted);
		System.out.println();
		
		System.out.println("binary==discrypted = "+(binary.compareTo(discrypted)==0));
		System.out.println();
		
		System.out.println("Valeur message = "+(DESUtil.AsciiDecoding(Binary.binaryStringToIntTable(discrypted))));
		System.out.println();		
	}
}
