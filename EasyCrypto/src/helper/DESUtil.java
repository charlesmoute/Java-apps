...
public class DESUtil {
	...
	public static String messageToDESBinaryString(String message) throws Exception{
		...
		if( (sizeMessage%Constant.SIXTYFOUR_BIT)!=0){
			int count = Constant.SIXTYFOUR_BIT - (sizeMessage%Constant.SIXTYFOUR_BIT) ;
	...
	public static final String AsciiDecoding(ArrayList<Integer> param) throws Exception{
		...
		for(int i=0;i<param.size();i++){
			...
			if(val<Constant.MIN_ASCII || val>Constant.MAX_ASCII) throw new Exception("La chaine a decoder n'est pas une chaine Ascii");
			buffer.put(val);
		}
		buffer.flip();
	...
	public static final ArrayList<Integer> AsciiEncoding(String param) throws Exception{
		...
		for(int i=0;i<param.length();i++) {
			...
			if(val<Constant.MIN_ASCII || val>Constant.MAX_ASCII) throw new Exception("La chaine a encoder n'est pas une chaine Ascii");
			result.add(val);
		}
		return result;		
	}
	...
	public static String buildKey(String param) throws Exception {
		String result = null;
		if(param!=null && param.length()>Constant.SIX_BIT){
			result = "";
			String str = Binary.intToBinaryString(DESUtil.AsciiEncoding(param.substring(0,7)));			
			if(str.length()!=Constant.FIFTYSIX_BIT) throw new Exception("La cle ne peut tenir sur 56 bits");		
			int i=0,incr = 7,count;			
			while(i<Constant.FIFTYSIX_BIT){
			...
			if(result.length()!=Constant.SIXTYFOUR_BIT) throw new Exception("Erreur Interne :: La cle generee ne tient pas sur 64 bits");
		}
		return result;
	}
	...
	public static boolean isDESReadableMessage(String message){
		if(message!=null && message.length()>0){
			...
			for(int i=0;i<message.length();i++){				
				 if(message.charAt(i)<Constant.MIN_ASCII || message.charAt(i)>Constant.MAX_ASCII){
					 isDESMessage = false ;
					 break;
				 }
			}
