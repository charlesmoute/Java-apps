...
public class RSAUtil {
	 ...
	 * @throws Exception Erreur due a un mauvaise execution du code.
	 */
	public static final  model.base.Key[] buildKeysOf(BigInteger p,BigInteger q) throws Exception {
		...
		model.base.Key[] keys = new model.base.Key[2];
		...
		BigInteger d = r[1]; 
		keys[0] = new model.base.Key(e,n);
		keys[1] = new model.base.Key(d,n);
		return keys;
	}
	/*public static final  model.base.Key[] buildKeysOf(long p,long q) throws Exception {
		...
		keys[0] = new model.base.Key(BigInteger.valueOf(e),BigInteger.valueOf(n));
		keys[1] = new model.base.Key(BigInteger.valueOf(d),BigInteger.valueOf(n));
		return keys;
	}*/
	...
	public static final BigInteger inverse (BigInteger a , BigInteger b ) {		 
	  ...
	  */
	public static final ArrayList<model.base.Number> decompose(BigInteger n) throws Exception {
		 
		if(n==null || n.compareTo(BigInteger.ZERO)<=0) throw new Exception("La decomposition de nombre <=0 n'est pas possible");
		 ArrayList<model.base.Number> res = new ArrayList<model.base.Number> (); 
		 if(n.compareTo(BigInteger.ONE)==0){
			 model.base.Number number = new model.base.Number(BigInteger.valueOf(2),0);
			 res.add(number);
			 return res;
		 }
		 ...
		 while(true){			 
			 ...
			 if(e>0){
				 model.base.Number number = new model.base.Number(prime,e);
				 res.add(number);
			 }			 
	 ...
	 /*public static final ArrayList<model.base.Number> decompose(long n) throws Exception {
		 ...
		 while(true){			 
			 ...
			 if(e>0){
				 model.base.Number number = new model.base.Number(BigInteger.valueOf(prime),e);
				 res.add(number);
			 }			 
			 prime++;
		 }
	 ...
	 public static final BigInteger phi(BigInteger n) throws Exception{
		 ArrayList<model.base.Number> factors = RSAUtil.decompose(n);
		 BigInteger phi = BigInteger.ONE ;
		 for(int i=0;i<factors.size();i++){
			 model.base.Number number = factors.get(i);
	...
	/* public static final long phi(long n) throws Exception{
	 ...
	 /**
	  * Trouve l'autre cle a partir de la cle, generalement cle publique, passe en parmetre
	  * @param key Cle publique
	  * @return Cle secrete
	  * @throws Exception Erreur generer lorsque le code ne se deroule pas correctement.
	  */
	 public static final model.base.Key findOtherKey(model.base.Key key) throws Exception {
		...
		return (new model.base.Key(RSAUtil.inverse(key.getExponent(), phi),key.getModulo())) ;			
	 }
	 /*public static final model.base.Key findOtherKey(model.base.Key key) throws Exception {
		if(key==null) throw new Exception("La cle est incorrecte");
	 ...
	 public static String numberToString(BigInteger number) throws Exception {
		 ...
		 while(begin<param.length()){
			 ...
			 if(value<Constant.MIN_ASCII || value>Constant.MAX_ASCII)  throw new Exception("L'entier ne peut etre converti en chaine de caractere ASCii");
			 result+=((char)value);
			 begin+=incr;
		 } 
		 return result;
	...
	public static void main(String[] arg) throws Exception {
		...
		model.base.Key[] keys = RSAUtil.buildKeysOf(primes[0], primes[1]);
		...
		model.ciphering.RSA rsa = new model.ciphering.RSA(keys[0],keys[1]);
		...
		ArrayList<model.base.Number> res = RSAUtil.decompose(factor);
		...
		model.base.Key key = RSAUtil.findOtherKey(new model.base.Key(e,n));		
