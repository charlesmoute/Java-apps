 ...
 */
public class ConnectionModel extends SuperModel {
	...
	private ActionListener refreshTask = new ActionListener(){
		...
		public void actionPerformed(ActionEvent evt){
			EntryModel m = ConnectionModel.this.getSubModel(EntryModel.class);
			...
			if(!system.entryIsActive(entryName)){
				...
				ConnectionModel.this.setState(ConnectionConstants.INACTIVE_STATE, 0);
			}else{
	...
	private JDNotification connectionNotification = new JDNotification (){
		...
		public void notify(String entry, int state, int error) {
			...
			ConnectionModel.this.currentState = state;
			/*
			...
			if(state==JDState.DIAL_TIMEOUT)	ConnectionModel.this.setMessage("Dial Time out") ;
			else ConnectionModel.this.setMessage(JDState.getStateString(state));
				...
				public void run(){
					if(status==JDState.Ras_Connected || code!=0){	
						if(status==JDState.Ras_Connected){
							...
							ConnectionModel.this.setState(ConnectionConstants.ACTIVE_STATE, 0);
							horloge.start();
						}else{
							//ici affecte l'etat de l'entre a disconnect
							ConnectionModel.this.setState(ConnectionConstants.INACTIVE_STATE, 0);
						}
						ConnectionModel.this.setMessage("");
					}
				}
			});
	 ...
	 */
	public ConnectionModel() throws ProgramException{
		this(null,null);
	}
	...
	public ConnectionModel(String utilisateur,String password) throws ProgramException{
	...
	private static List<ConnectionModel> connections = new ArrayList<ConnectionModel>();
	{
